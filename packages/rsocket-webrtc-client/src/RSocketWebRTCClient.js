...
export type ClientOptions = {|
  rtcDataChannelCreator: (RTCDataChannelInit) => Promise<RTCDataChannel>,
  debug?: boolean,
 ...
 */
export default class RSocketWebRTCClient implements DuplexConnection {
  _encoders: ?Encoders<*>;
  ...
  connect(): void {
    invariant(
      ...
      'RSocketWebRTCClient: Cannot connect(), a connection is already ' +
        'established.',
    );
    this._setConnectionStatus(CONNECTION_STATUS.CONNECTING);

    const rtcDataChannelCreator = this._options.rtcDataChannelCreator;
    rtcDataChannelCreator({ordered: true}).then(
      channel => {
        this._channel = channel;
        channel.binaryType = 'arraybuffer';
        (channel.addEventListener: $FlowIssue)('close', this._handleClosed);
        (channel.addEventListener: $FlowIssue)('error', this._handleError);
        // FIXME: for some reasons does not work with node-webrtc
        // (channel.addEventListener: $FlowIssue)('open', this._handleOpened);
        (channel.addEventListener: $FlowIssue)('message', this._handleMessage);
        this._handleOpened();
      },
      error => {
